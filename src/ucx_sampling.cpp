...
int ucx_sampling::ucx_statistics_server_start(int port)
{
    ...
    std::cout << "ucx_statistics_server_start()" << std::endl;
    std::cout << "UCX Port used = \n" << port << std::endl;

    m_statistics_server_process_enable = 0;
    status = ucs_stats_server_start(port, &m_ucx_stats_server);
    if (status != UCS_OK) {
        std::cout << "ucs_stats_server_start() Failed! status=" << status << "\n";
        ret = -1;
        goto exit_and_return_metric_properties;
    }

    std::cout << "ucs_stats_server_start() Succes! status=" << status << "\n";
...
#if 0
    /* Statistics server runs only at root */
    if (mpi_rank != 0) {
        return 0;
    }
#endif
